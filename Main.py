...
while True:
    ...
    if faceCurFrame: 
        ...
        if counter != 0:
            if counter == 1:
                ...
                img_path = os.path.join(student_images_dir, f"{id}.png")
                if os.path.exists(img_path):
                    imgStudent = cv2.imread(img_path)
                else:
                    imgStudent = np.zeros((216, 216, 3), dtype=np.uint8)  # Placeholder for missing image
    ...
    # Exit condition: If "Esc" key is pressed, break the loop and release the camera
